...
export default function NotesScreen() {
  ...
  const onRefresh = () => { setRefreshing(true); loadNotes(true); };
  ...
  // FILTRAGE
  const notesFiltrees = filtreActif === 'Tous'
    ? notes
    : notes.filter(n => (n.Lib_Sem || 'N/A') === filtreActif);
        ...
        {moyenne && (
          <View style={styles.moyenneCard}>
            <View style={styles.moyenneRow}>
              <Text style={styles.moyenneTitre}>Moyenne générale</Text>
              <Text style={[styles.moyenneVal, { color: getMoyenneColor(moyenne) }]}>{moyenne}/20</Text>
            </View>
            <View style={styles.progressBg}>
              <View style={[styles.progressFill, {
                width: `${(parseFloat(moyenne) / 20) * 100}%`,
                backgroundColor: getMoyenneColor(moyenne)
              }]} />
            </View>
        ...
        {semestres.length > 1 && (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filtresScroll} contentContainerStyle={styles.filtresContent}>
            {semestres.map(sem => (
              <TouchableOpacity
                key={sem}
                style={[styles.filtre, filtreActif === sem && styles.filtreActif]}
                onPress={() => setFiltreActif(sem)}
              >
                <Text style={[styles.filtreTxt, filtreActif === sem && styles.filtreTxtActif]}>{sem}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        )}
        ...
        <View style={styles.listContainer}>
          {notesFiltrees.length === 0 ? (
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyTxt}>Aucune note pour ce semestre</Text>
            </View>
          ) : (
            notesFiltrees.map((note, index) => {
              ...
              return (
                <View key={index} style={[styles.card, { borderLeftColor: colors.border }]}>
                  ...
                  {/* BARRE DE NOTE */}
                  <View style={styles.noteBarBg}>
                    <View style={[styles.noteBarFill, {
                      width: `${(n / 20) * 100}%`,
                  ...
                  <View style={styles.cardBottom}>
                    <Text style={styles.cardInfo}>📅 {note.Date_Evaluation ? new Date(note.Date_Evaluation).toLocaleDateString('fr-FR') : 'N/A'}</Text>
                    <Text style={styles.cardInfo}>📚 {note.Lib_Sem || 'N/A'}</Text>
                  </View>
                </View>
              );
            })
...
const styles = StyleSheet.create({
  ...
  // HERO
  hero: {
    backgroundColor: '#0F172A', paddingTop: 20,
  ...
  decoCircle1: {
    ...
    backgroundColor: 'rgba(37,99,235,0.12)', top: -50, right: -30,
  },
  decoCircle2: {
    position: 'absolute', width: 120, height: 120, borderRadius: 60,
    backgroundColor: 'rgba(37,99,235,0.08)', bottom: -20, left: -20,
  },
  ...
  moyenneTitre: { fontSize: 14, fontWeight: '700', color: '#1E293B' },
  moyenneVal: { fontSize: 22, fontWeight: '900' },
  progressBg: { height: 8, backgroundColor: '#F1F5F9', borderRadius: 8, overflow: 'hidden' },
  progressFill: { height: '100%', borderRadius: 8 },
  moyenneLabel: { fontSize: 12, color: '#64748B', marginTop: 6, textAlign: 'right' },
  ...
  matiere: { fontSize: 15, fontWeight: '800', color: '#1E293B', marginBottom: 6 },
  ...
  noteBarBg: { height: 5, backgroundColor: '#F1F5F9', borderRadius: 5, overflow: 'hidden', marginBottom: 10 },
